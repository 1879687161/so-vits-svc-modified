...
class TTS(object):
    ...
    def inference_part(self):
        ...
        for clean_name, tran in zip(self.clean_names, self.trans):
            ...
            for spk in spk_list:
                ...
                if use_spk_mix:
                    spk = "spk_mix"
                res_path = f'results/{clean_name}_{key}_{spk}{cluster_name}_{isdiffusion}_{self.f0p}.{self.wav_format}'
                soundfile.write(res_path, audio, svc_model.target_sample, format=self.wav_format)
                svc_model.clear_empty()
    
    def tts_func(self, _text, _rate, _voice):
        #使用edge-tts把文字转成音频
        ...
        output_file = "C:/Users/zhangkaihao/so-vits-svc/raw/audio.wav"
    ...
    def total_inference(self, TEXT = "请在这里输入文字"):
        self.tts_func(TEXT, self.rate, self.voice)
        self.inference_part()